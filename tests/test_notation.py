...
class TestRewriteODEScalar:
    ...
    def test_bare_f(self):
        assert rewrite_f_expression("f", self.nota) == "y[0]"

    def test_f_zero(self):
        assert rewrite_f_expression("f[0]", self.nota) == "y[0]"

    def test_f_one(self):
        assert rewrite_f_expression("f[1]", self.nota) == "y[1]"

    def test_expression(self):
        result = rewrite_f_expression("-omega**2 * f[0]", self.nota)
        assert result == "-omega**2 * y[0]"

    def test_multiple_refs(self):
        result = rewrite_f_expression("-2*gamma*f[1] - omega**2*f[0]", self.nota)
        assert result == "-2*gamma*y[1] - omega**2*y[0]"

    def test_bare_f_in_expression(self):
        result = rewrite_f_expression("k * f", self.nota)
        assert result == "k * y[0]"

    def test_no_match_function_names(self):
        """Should not rewrite 'floor', 'float', etc."""
        result = rewrite_f_expression("floor(f[0])", self.nota)
        assert result == "floor(y[0])"

    def test_no_match_function_call(self):
        """f followed by ( is a function call, not our f variable."""
        result = rewrite_f_expression("func(x)", self.nota)
        assert result == "func(x)"

    def test_empty_brackets(self):
        result = rewrite_f_expression("f[]", self.nota)
        assert result == "y[0]"


# ── rewrite_f_expression: Difference ──────────────────────────────────
...
class TestRewriteDifference:
    ...
    def test_expression(self):
        result = rewrite_f_expression("f[1] + f[0]", self.nota)
        assert result == "y[1] + y[0]"


# ── rewrite_f_expression: Vector ODE ─────────────────────────────────
...
class TestRewriteVectorODE:
    ...
    def test_f_ik_literal(self):
        assert rewrite_f_expression("f[0,0]", self.nota) == "y[0]"
        assert rewrite_f_expression("f[0,1]", self.nota) == "y[1]"
        assert rewrite_f_expression("f[1,0]", self.nota) == "y[2]"
        assert rewrite_f_expression("f[1,1]", self.nota) == "y[3]"
        assert rewrite_f_expression("f[2,0]", self.nota) == "y[4]"
        assert rewrite_f_expression("f[2,1]", self.nota) == "y[5]"

    def test_single_index_implicit_component(self):
        """f[k] with single index -> y[k] (component 0 implicit)."""
        result = rewrite_f_expression("f[1]", self.nota)
        assert result == "y[1]"
    ...
    def test_symbolic_indices(self):
        ...
        result = rewrite_f_expression("f[i,0]", self.nota)
        assert result == "y[(i)*2+(0)]"

    def test_modular_symbolic(self):
        """f[(i+1)%n,0] -> y[((i+1)%n)*2+(0)]."""
        result = rewrite_f_expression("f[(i+1)%n,0]", self.nota)
        assert result == "y[((i+1)%n)*2+(0)]"


# ── rewrite_f_expression: Heterogeneous orders ───────────────────────
...
class TestRewriteHeterogeneousOrders:
    ...
    def test_f02(self):
        assert rewrite_f_expression("f[0,2]", self.nota) == "y[2]"

    def test_f10(self):
        # Component 1 starts at offset 3 (sum of component_orders[:1])
        assert rewrite_f_expression("f[1,0]", self.nota) == "y[3]"

    def test_f11(self):
        assert rewrite_f_expression("f[1,1]", self.nota) == "y[4]"
...
class TestFlatIndexToLabel:
    ...
    def test_vector_ode(self):
        ...
        assert flat_index_to_label(1, nota) == "f\u2032\u2080"
        assert flat_index_to_label(2, nota) == "f\u2081"
        assert flat_index_to_label(3, nota) == "f\u2032\u2081"
