...
def solve_ode_bvp(
    ...
    """Solve a boundary-value ODE problem using ``scipy.integrate.solve_bvp``.

    Args:
        ode_func: Right-hand side ``dy/dx = f(x, y)``.
        bc_func: Boundary condition residuals ``bc(ya, yb) = 0``.
        x_mesh: Initial mesh of x-values.
        y_guess: Initial guess for y on the mesh — shape ``(n_vars, n_mesh)``.
    ...
    Raises:
        SolverFailedError: If the solver reports failure.
    """
    logger.info("Solving BVP: mesh size=%d, tol=%s", len(x_mesh), tol)
    ...
    result = ODESolution(
        ...
        y=sol.y,
        success=sol.success,
        message=sol.message if hasattr(sol, "message") else ("OK" if sol.success else "Failed"),
        method_used="BVP (collocation)",
        n_eval=getattr(sol, "niter", 0),
        raw=sol,
    )

    if not sol.success:
        msg = getattr(sol, "message", "BVP solver failed")
        logger.error("BVP solver failed: %s", msg)
        raise SolverFailedError(f"BVP solver failed: {msg}")

    logger.info("BVP solver succeeded: %d mesh points", len(sol.x))
    return result
